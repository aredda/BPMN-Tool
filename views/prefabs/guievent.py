...
class GUIEvent(GUILinkable):
    ...
    def draw_at(self, x, y):
        ...
        # draw borders
        cnv.create_oval(x, y, x + self.PERIMETER, y + self.PERIMETER, fill=cnv['bg'], outline=black, width=borderwidth)
        # draw inner border
        if eventtype in [EventType.IntermediateCatch, EventType.IntermediateThrow]:
            cnv.create_oval(x + 4, y + 4, x + self.PERIMETER - 4, y + self.PERIMETER - 4, fill=cnv['bg'], outline=black, width=borderwidth)
